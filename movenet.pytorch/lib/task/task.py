...
################ 
    def onTrainStep(self,train_loader, epoch):
        ...
        for batch_idx, (imgs, labels, kps_mask,img_names) in enumerate(train_loader):
            ...
            if batch_idx%self.cfg['log_interval']==0:
                ...
                #Crea un array con le variabili
                data_array.append(total_loss.item())
                data_acc.append(np.mean(right_count/total_count))
                # Specifica il percorso del file di esportazione
                output_file = "./data/loss_train/"+str(epoch+1)+".json"
                output_file_acc = "./data/acc_train/"+str(epoch+1)+".json"
                # Serializza l'array in formato JSON
                json_data = json.dumps(data_array)
                json_data_acc = json.dumps(data_acc)

                # Scrivi il JSON nel file
                if not os.path.exists('./data/loss_train'):
                    os.makedirs('./data/loss_train')
                with open(output_file, "w") as file:
                    file.write(json_data)
                if not os.path.exists('./data/acc_train'):
                    os.makedirs('./data/acc_train')
                with open(output_file_acc, "w") as file:
                    file.write(json_data_acc)

                # Stampa un messaggio di conferma
                print("Array salvato nel file", output_file)
                print("Array salvato nel file", output_file_acc)
    ...
    def onValidation(self, val_loader, epoch):
        ...
        with torch.no_grad():
            ...
            # Specifica il percorso del file di esportazione
            output_file = "./data/loss_val/"+str(epoch+1)+".json"
            output_file_acc = "./data/acc_val/"+str(epoch+1)+".json"
            ...
            # Scrivi il JSON nel file
            if not os.path.exists('./data/loss_val'):
                    os.makedirs('./data/loss_val')
            with open(output_file, "w") as file:
                file.write(json_data)
            if not os.path.exists('./data/acc_val'):
                    os.makedirs('./data/acc_val')
            with open(output_file_acc, "w") as file:
                file.write(json_data_acc)
    ...
    def onTest(self):
     ...
     #self.model = net.load_state_dict(torch.load('classifier.pt', map_location=torch.device('cpu')))
